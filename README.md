...
ObfSymbolsEx uses a novel approach to access the DIA SDK:

1. **Dynamic Loading**: Loads `msdia140.dll` using `LoadLibrary` (no registration)
2. **Smart Search**: Checks executable directory first, then Visual Studio installations, then `%PATH%`
...
See `ObfSymbolsEx/IMPLEMENTATION_NOTES.md` for detailed technical information.
...
```cmd
"$(SolutionDir)ObfSymbolsEx\x64\Release\ObfSymbolsEx.exe" "$(TargetDir)$(TargetName).pdb" "$(TargetDir)$(TargetName)_symbols.sym"
```
